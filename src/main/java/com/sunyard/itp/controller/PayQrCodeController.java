...
public class PayQrCodeController {
	...
	@ResponseBody
	public String createQr(HttpServletResponse resp,String totalFee){
		try {
			payQrCodeService.createQr(resp,totalFee);
		} catch (Exception e) {
			logger.debug(e.getMessage());
			return CommonConst.getResult(CommonConst.RET_ERROR,"系统错误");
		}
		return CommonConst.getResult(CommonConst.RET_SUCCESS,"动态二维码生成成功！");
	}
